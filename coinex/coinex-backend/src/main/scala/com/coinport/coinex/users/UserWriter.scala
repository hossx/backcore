...
class UserWriter(db: MongoDB, secret: String) extends ExtendedView {
  ...
  def updateState(event: Any) = event match {
    ...
    case DoRequestPasswordReset(email, token, _, _) => profiles.put(manager.requestPasswordReset(email, token.get))
    ...
    case CleanUserData(actions) => manager.cleanData(actions).foreach(profiles.put(_))
    case _ =>
  }
